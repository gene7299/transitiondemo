...
$.fn.roulette = function(options) {
      ...
      var turnplate = $.extend({
            ...
            textRadius:r_width/2.5272,				//大转盘奖品位置距离圆心的距离
...
//background:#e62d2d;
        ...
        turnplate.restaraunts = ["Five Star||Dinner||for FREE", "Coupon||$100", "All day SPA || package", "Coupon||$200", "FREE car||rental", "Africa||Safari||Trip", "Coupon||$500", "SORRY", "7 day||cruise", "Coupon||$300"];
        ...
        function drawRouletteWheel() {
          ...
          if (canvas.getContext) {
        	  ...
        	  for(var i = 0; i < turnplate.restaraunts.length; i++) {
        		  ...
        		  if(false && text.indexOf("M")>0){//流量包
        			  var texts = text.split("M");
        			  for(var j = 0; j<texts.length; j++){
        				  ctx.font = j == 0?'bold '+r_font+'px Microsoft YaHei':r_font+'px Microsoft YaHei';
        				  if(j == 0){
        		  ...
        		  }else if(false && text.length>8){//奖品名称长度超过一定范围
                ...
                var texts = text.split("||");
        		  ...
        		  //添加对应图标
        		  if(text.indexOf("禮券")>0 || text.indexOf("Coupon")>=0){
        			  var img= document.getElementById("shan-img");
        			  img.onload=function(){
        				  ctx.drawImage(img,-20,r_font*2);
        			  };
        			  ctx.drawImage(img,-20,r_font*1.77);
        		  }else if(text.indexOf("谢谢参与")>=0 || text.indexOf("銘謝惠顧")>=0  || text.indexOf("SORRY")>=0 ){
