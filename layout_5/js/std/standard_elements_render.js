...
var setjson = null; //call by android
...
*/
	...
	//setTimeout(function(){
		...
		//start MRT element
		var Param_stationOrder = 1;
		if (getParameterFromURL(document.location.href, 'stationOrder') == '0') {
				Param_stationOrder = 0;
				console.log('DEFINE_stationOrder=' + Param_stationOrder);
		}
		console.log(unescape(mapjson));
		//routemaker1 = _routemaker('route1').mapjson(unescape(mapjson)).stationOrder(Param_stationOrder);
    //routemaker1 = _routemaker('route1').pagewidth(XviewportH).pageheight(XviewportV).mapjson(unescape(mapjson)).stationOrder(Param_stationOrder);
    //routemaker1();
    //setjson = routemaker1.run;
		maprunner = _maprunner().mapjson(unescape(mapjson));
		maprunner();
		function setJsonToAll(json){
			maprunner.run(json);
		}
		setjson = setJsonToAll;
		//initSizeLab();
		if(typeof doAndroidNativeRender != "undefined"){console.log("go go Android Native!");doAndroidNativeRender(starttime);}

		var countToSimulate = 0;
		document.body.addEventListener('keydown', function (evt) {
      console.log('evt.keyCode='+evt.keyCode);
			console.log('countToSimulate='+countToSimulate);
      if (evt.keyCode === 72) {
        countToSimulate++;
        if(countToSimulate===5){
          setstandby = null;
          isNowStandby = false;
          maprunner.simulateRunning("R", "up_toward");
        }
      } else if (evt.keyCode === 82) {
        location.href = location.href;
      }

	  if(typeof Android != "undefined" && typeof Android.setKey != "undefined")
			Android.setKey(evt.keyCode);
    });
...
var stdRender = {
	...
	setMedia : function(obj){
      ...
      if(textProperties!=undefined)
      {
		...
		if(type=="mrt")
		{
			var mrtProperties = obj.jobj.mrtProperties;
			var mrtOptions = {};
			mrtOptions.state  = mrtProperties.state;
			mrtOptions.lang = mrtProperties.lang;

  		//$('#'+obj.htmlid).html("<span id='statictext-"+obj.id+"_"+obj.mId+"' class='statictext '>"+obj.ctx+"</span>");
			var text = '<span class="stationName" id="mrt_'+mrtOptions.state+'_'+mrtOptions.lang+'">'+ decodeURIComponent(decodeURIComponent(obj.ctx)) + '</span>';
			$('#'+obj.htmlid).html(text);
			$('#'+obj.htmlid).addClass('statictext');
			$('#'+obj.htmlid).attr('type','text');
			//$('#'+obj.htmlid).html();
