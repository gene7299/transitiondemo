! function() {
	...
	$.fn.breakingNews = function(bn_options) {
		var option = {
			...
			if(temp.attr("fontSize")!=undefined){ttitle.css('font-size',temp.attr("fontSize")*1.25);}
			...
			if(temp.attr("fontSize")!=undefined){tul.css('font-size',temp.attr("fontSize"));}
			...
			function loadRssFeedsViaGoogle() {
				...
				for (rssList = l.feed.split(","), /*labelList = l.feedlabels.split(","),*/ len = 0, l.modul.find("ul").html(""), xx = 0, k = 0; k < rssList.length; k++) $.ajax({
					type: "GET",
